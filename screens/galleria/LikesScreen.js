...
const LikesScreen = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_URL}/api/posts/${id}/likes/?name=${searchText}&page=${page}`, config);
  ...
  return (
    <View style={styles.screen}>
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <Text onPress={handleRefresh} style={{ fontSize: 20, fontWeight: 'bold', color: 'red' }}>My Liked Posts </Text>
        <Ionicons name="heart-sharp" size={24} color="red" style={{ marginLeft: 8 }} />
      </View>
      ...
      {loading && posts.length == 0 ? (
        ...
        <View style={styles.noPostsContainer}>
          <Text style={styles.noPostsText}>
            It seems you have no liked Posts Yet
          </Text>
